...
class Track3D extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (!this.state.rendererInitialized && nextState.rendererInitialized)
      return true;
    return false;
  }
  ...
  render() {
    return (
      <div ref={this.rendererContainer} className={styles.rendererContainer}>
        {!this.webglAvailable ? (
          <div
            dangerouslySetInnerHTML={{
              __html: WEBGL.getWebGLErrorMessage().innerHTML,
            }}
          ></div>
        ) : null}
