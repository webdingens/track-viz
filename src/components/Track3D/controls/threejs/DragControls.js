...
var DragControls = function ( _objects, _camera, _domElement ) {
	...
	function _moveSelectedToCursor( event ) {
		...
		if ( _selected && scope.enabled ) {
			if ( _raycaster.ray.intersectPlane( _plane, _intersection ) ) {
				...
				let _selectedIntersectionCurrent = _intersection.clone().add(invDir.clone().multiplyScalar(factorToSelectedIntersection));

				_selected.position.copy( _selectedIntersectionCurrent.sub(_selectedIntersectionOffset) );
	...
	function _selectFromCursor(event) {
		...
		if ( _intersections.length > 0 ) {
			...
			_selectedIntersectionOffset = _selectedIntersectionOffset.copy(_selectedIntersection.point).sub(_selected.position);
