...
import { selectTrack3DUseTextures } from "../../app/reducers/settingsTrack3DSlice";
...
class Track3DFloor extends React.PureComponent {
  ...
  componentWillUnmount() {
    if (this.props.scene && this.floor) this.props.scene.remove(this.floor);
  }
  ...
  loadTexturedMaterial() {
    this.getTexturedMaterial().then((material) => {
      ...
      this.floor.material.map.repeat.set(0.1 * 35, 0.1 * 25);
          ...
          for (let key in images) {
            loadingImages.push(
              new Promise((resolve, reject) => {
                ...
                loader.load(asphaltFolder + "/" + images[key], (texture) => {
                  texture.wrapS = THREE.RepeatWrapping;
                  texture.wrapT = THREE.RepeatWrapping;
                  texture.repeat.set(5, 5);
                  texture.anisotropy = Math.min(
                    2,
                    this.props.renderer.capabilities.getMaxAnisotropy()
                  );
